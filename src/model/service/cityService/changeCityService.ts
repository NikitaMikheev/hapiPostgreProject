...
export const cityServiceChange = async (id:number, payload:myUser, newUser:User):Promise<void> => {
    ...
    const res = await users.find({ // обязательно подгружаем связи !!!!!!!
        where: { id },
        relations: {
            city_user: true
        }
    ...
    if(city) { // такой город уже в базе данных, значит пушим юзера 
